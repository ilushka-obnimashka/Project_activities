...
@click.argument('output_path', default='processed_video.mp4')
@click.option('--noise', 'noise_type', type=click.Choice(['salt_pepper', 'gaussian', 'none'], case_sensitive=False), default='salt_pepper', help='Type of noise to add.')
...
def main(input_path, output_path, noise_type, salt_prob, pepper_prob, mean, sigma):
    """
    Processes a video stream, adds specified noise, and applies an adaptive median filter.
    
    INPUT_PATH: Path to video file or camera ID (e.g., "0").
    OUTPUT_PATH: Path to save the processed video (ignored for webcam).
    """
    
    # Assemble noise parameters
    noise_params = {}
    ...
    try:
        processor = NoiseFilterProcessor(
